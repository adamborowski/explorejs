...
const MergeOperation = require("../../src/layered/MergeOperation");
...
function perform(config) {
    const F = rng(config.F);
    const T = rng(config.T);
    const B = rng(config.B);
    const R = rng(config.R);
    let rangeDrawing = TestUtil.getRangeDrawing([F, T, B, R], 'FTBR');
    // console.log('\n\n>> Layer merge diff test\n');
    // console.log(rangeDrawing);
    const diff = MergeOperation.execute(B, T, F, R);
    let resultDrawing = TestUtil.getRangeDrawing([F, diff.T.result, diff.B.result, diff.R.result], 'FTBR');
...
describe('MergeOperation', ()=> {
    ...
    describe('Randomized tests', ()=> {
        const numRandomizedTests = 500;
        const rand = gen.create("MergeOperation randomized test");
        for (let i = 0; i < numRandomizedTests; i++) {
            it('random #' + i, ()=> {

                // prepare Front, Target and Back layers
                const setSize = rand.intBetween(0, 30);
                const rangeSet = TestUtil.randomRangeSet(setSize, rand, () => rand.floatBetween(0, 1) > 0.3 ? 0 : rand.intBetween(1, 10), {
                    start: 1,
                    end: 7
                });
                const F = [], T = [], B = [], R = [], sets = [F, T, B, R];
                const randomSwitcher = TestUtil.getSwitcher(rand, 3);
                for (let range of rangeSet) {
                    sets[randomSwitcher.next()].push(range);
                }

                // prepare merge operand
                const margin = rand.intBetween(-5, 5);
                ...
                const T2_cmp = xspans.union(T, R).subtract(F).toObjects("start", "end");
                const B2_cmp = xspans.subtract(B, T2_cmp).subtract(F).toObjects("start", "end");
                const R2_cmp = xspans.subtract(R, F).toObjects("start", "end");
