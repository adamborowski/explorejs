...
class ParallelRangeSetIterator {
    ...
    next() {
        this.counter++;
        const moveLeftRequested = this._moveLeftRequested;
        const moveRightRequested = this._moveRightRequested;
        let suspendLeftRequested = this._suspendLeftRequested;
        let suspendRightRequested = this._suspendRightRequested;
        ...
        const nextLeft = this._peekNextLeft();
        const nextRight = this._peekNextRight();
        ...
        let leftPoint = Infinity, rightPoint = Infinity;
    ...
    insertAsNextLeft(range) {
        const nextRegularLeft = this._leftSet[this._iLeft + 1];
        const insertionPoint = this._leftBuffer.length === 0 ? this._left : this._leftBuffer[this._leftBuffer.length];
